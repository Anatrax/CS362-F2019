...
void testIncreasesBuysBy1()
{
  ...
  baronCard(-1, &G, currentPlayer); // Call function
...
void testDiscardingEstateIncreasesPlayersCoinsBy4()
{
  ...
  baronCard(discardEstate, &G, currentPlayer);
...
void testGainCard()
{
  ...
  baronCard(gainEstate, &G, currentPlayer);
...
void test4()
{
  ...
  // Call function
  result = baronCard(discardEstate, &G, currentPlayer);

  // Assert that the removed index is set to -1
  // assertEqual(G.hand[currentPlayer][estateIndex], -1);
  assertEqual(G.hand[currentPlayer][G.handCount[currentPlayer] + 1], -1);
}
