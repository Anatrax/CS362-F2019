...
int assertNotEqual(char *fail_str, int value, int expected_value)
{
  if(value == expected_value)
  {
    printf("ERROR: %s\n", fail_str);
...
int assertLessThan(char *fail_str, int value, int expected_value)
{
  if(value >= expected_value)
  {
    printf("ERROR: %s\n", fail_str);
...
int checkExpectedMinionState(struct gameState *prev, struct gameState *cur, int handPos, int currentPlayer, int gainCoins, int redraw)
{
  ...
  prev->playedCards[prev->playedCardCount] = minion;
  ...
  if(gainCoins)
  {
    // Player gains two coins
    prev->coins+=2;
    if(!assertEquals("hands modified",compare2DArray(prev->hand,prev->numPlayers,prev->handCount,cur->hand,cur->numPlayers,cur->handCount),0))
      return 1;
  ...
  {
    // Player has 4 cards
    prev->handCount[currentPlayer] = 4;
    //for each player, check if handCount < 5
    int i;
    for(i = 0; i < prev->numPlayers; i++)
    {
      char *fail_str[32];
      sprintf(fail_str,"player %d's expected handCount is not < 5",i);
      if(!assertLessThan(fail_str,cur->handCount[i],5))
        return 1;
    }
  }
  else
  {
    // assertEquals("no option choosen",'n','a');
    // return 1;
  }
  ...
  if(!assertEquals("playedCardCount did not increase by one",prev->playedCardCount,cur->playedCardCount))
    return 1;
  // if(!assertEquals("playedCards top card is not minion",compareArray(prev->playedCards,prev->playedCardCount,cur->playedCards,cur->playedCardCount),0))
  //   return 1;
  if(!assertEquals("numActions did not increase by one",prev->numActions,cur->numActions))
    return 1;
  if(!assertEquals("expected coins doest not match output",prev->coins,cur->coins))
    return 1;
  if(!assertEquals("expected handCount does not match output",prev->handCount[currentPlayer],cur->handCount[currentPlayer]))
    return 1;
  return 0;
}
...
int minionRandomTest(struct gameState *state)
{
  randGameState(state);
  int currentPlayer = randRanged(0,4);
  int handPos = randRanged(0,state->handCount[currentPlayer]);
  ...
  int result = playMinion(state, handPos, currentPlayer, gainCoins, redraw);
  if(result != 0){
    printf("Function Returned Non-Zero Value: %d\n",result);
    // TODO: assert if failure was expected
    return 1;
  }
  else if (checkExpectedMinionState(&previousState,state,handPos,currentPlayer,gainCoins,redraw) != 0) // needs to be deep comparison
  {
    printf("Test Failed:\n");
    printf("\tHand Position: %d\n", handPos);
    printf("\tCurrent Player: %d\n", currentPlayer);
    printf("\tGain 2 Coins: %d\n", gainCoins);
    printf("\tRedraw 4 Cards: %d\n", redraw);
