...
int checkExpectedTributeState(struct gameState *prev, struct gameState *cur, int currentPlayer, int nextPlayer, int tributeRevealedCards[])
{
...
int tributeRandomTest(struct gameState *state)
{
  ...
  int result = playTribute(state, currentPlayer, nextPlayer, tributeRevealedCards);
  if(result != 0){
    printf("Function Returned Non-Zero Value: %d\n",result);
    // TODO: assert if failure was expected
    return 1;
  }
  else if (checkExpectedTributeState(&previousState,state,currentPlayer,nextPlayer,tributeRevealedCards) == 0) // needs to be deep comparison
  {
    printf("Success!");
  }
  else
