...
{
  if(arr1_depth != arr2_depth) return 1;
  int i;
  for(i = 0; i < arr1_depth; i++)
  {
    if(compareArray(&arr1[i], arr1_lengths[i], &arr2[i], arr2_lengths[i]) != 0) return 1;
  }
  return 0;
}

int checkExpectedBaronState(struct gameState *prev, struct gameState *cur, int discardEstate, int currentPlayer)
{
  ...
  if(!assertEquals("playedCards modified",compareArray(prev->playedCards,prev->playedCardCount,cur->playedCards,cur->playedCardCount),0))
    return 1;
  if(!assertEquals("other players' discard cards modified",compare2DArray(&(prev->discard[1]),prev->numPlayers-1,&(prev->discardCount[1]),&(cur->discard[1]),cur->numPlayers-1,&(prev->discardCount[1])),0))
    return 1;
  if(!assertEquals("player's other discard cards modified",compareArray(prev->discard[currentPlayer],prev->discardCount[currentPlayer],cur->discard[currentPlayer],prev->discardCount[currentPlayer]),0))
    return 1;
  if(!assertEquals("deck modified",compare2DArray(prev->deck,prev->numPlayers,prev->deckCount,cur->deck,cur->numPlayers,cur->deckCount),0))
    return 1;
  ...
  int curEstates = 0;
  int i;
  for(i = 0; i < cur->handCount[currentPlayer]; i++)
  {
    if(prev->hand[currentPlayer][i] == estate)
      prevEstates++;
    if(prev->hand[currentPlayer][i] == estate)
      curEstates++;
  }
  ...
  prev->playedCards[prev->playedCardCount] = baron;
  prev->playedCardCount++;
  prev->numBuys++;
  if(discardEstate)
  {
    // Only discards estate if there are estates to discard
    if(prevEstates > 0)
    {
      // Player discards a card
      prev->handCount[currentPlayer]--;

      // Player has one less estate card
      prevEstates--;

      // Discard pile gains a card
      prev->discardCount[currentPlayer]++;

      // Check that the top card on discard pile is estate
      if(!assertEquals("discard top card is not estate",cur->discard[currentPlayer][cur->discardCount[currentPlayer]],estate))
        return 1;

      // Player gained 4 coin
      prev->coins+=4;
    }
  }
  if(!discardEstate)
  {
    // Only gain estate if there are estates to gain
    if(prev->supplyCount[estate] > 0)
    {
      // Player gains a card
      prev->handCount[currentPlayer]++;

      // Player has one more estate card
      prevEstates++;

      // Supply loses an estate
      prev->supplyCount[estate]--;
    }
  }
  // Check that the previous state now matches the current state
  if(!assertEquals("playedCardCount did not increase by one",prev->playedCardCount,cur->playedCardCount))
    return 1;
  if(!assertEquals("playedCards top card is not baron",compareArray(prev->playedCards,prev->playedCardCount,cur->playedCards,cur->playedCardCount),0))
    return 1;
  if(!assertEquals("numBuys did not increase by one",prev->numBuys,cur->numBuys))
    return 1;
  if(!assertEquals("expected supplyCount doesn't match output",compareArray(prev->supplyCount,treasure_map+1,cur->supplyCount,treasure_map+1),0))
    return 1;
  if(!assertEquals("expected handCount doesn't match output",prev->handCount[currentPlayer],cur->handCount[currentPlayer]))
    return 1;
  if(!assertEquals("expected discardCount doesn't match output",prev->discardCount[currentPlayer],cur->discardCount[currentPlayer]))
    return 1;
  if(!assertEquals("expected number of estates in player's hand doesn't match output",prevEstates,curEstates))
    return 1;
...
{
  if(min_element > max_element) return;
  int i = 0;
  for(;i<size;i++)
  {
...
int baronRandomTest(struct gameState *state)
{
  randGameState(state);
  ...
  // Call Baron function
  int result = playBaron(state, discardEstate, currentPlayer);
  if(result != 0){
    printf("Function Returned Non-Zero Value: %d\n",result);
    // TODO: assert if failure was expected
    return 1;
  }
  else if (checkExpectedBaronState(&previousState,state,discardEstate,currentPlayer) != 0) // needs to be deep comparison
  {
    printf("Test Failed:\n");
    printf("\tDiscard Estate: %d\n", discardEstate);
